...
"radial-gradient(circle at 20% 30%, rgba(0,0,0,0.08) 0, transparent 50%), radial-gradient(circle at 80% 70%, rgba(0,0,0,0.05) 0, transparent 60%)",
 ...
 <div
 key={i}
 className="animate-track absolute border border-saffron/70"
 style={{
 left: `${b.x}%`,
 top: `${b.y}%`,
 width: `${b.w}%`,
 ...
 <div className="absolute -top-3 left-0 bg-saffron/90 px-1 py-[1px] text-[6px] font-semibold text-white whitespace-nowrap">
 ID:{idx}
 {i}
 </div>
 </div>
 ...
 <div className="absolute left-2 top-2 flex items-center gap-1.5 rounded-sm bg-surface/80 px-2 py-0.5 text-[9px] font-semibold text-foreground backdrop-blur-md ring-1 ring-border/50">
...
export function CCTVSection() {
 ...
 <div className="mt-0.5 text-[13px] font-semibold text-foreground leading-tight">
 {c.name}
 </div>
 </div>
 <span
 className={`inline-flex items-center rounded-xl border px-2 py-0.5 text-[9px] font-bold ${statusColor(c.status)}`}
 >
 {c.status}
 </span>
 </div>
 <div className="mt-2.5 flex items-center justify-between border-t border-border/50 pt-2.5 text-[11px] font-normal text-muted-foreground">
 ...
 <div className="mt-2 grid grid-cols-4 gap-1.5 opacity-0 transition-opacity group-hover:opacity-100">
 
 {[ZoomIn, Camera, Bell, Maximize2].map((Icon, j) => (
 <button
 key={j}
 className="flex h-7 items-center justify-center rounded-xl bg-surface text-muted-foreground transition-colors hover:bg-muted hover:text-foreground border border-border/50"
 >
 ...
 <span className={`font-semibold text-${a.color}`}>
 {a.cam}
 </span>
 ...
 <div className="mt-1 text-xs text-info font-normal">
 Tracking ID: SUB-8942-A
 </div>
 </div>
 {searchState ==="results" && (
 <button
 ...
 <div className="mb-8 flex items-center justify-between text-sm font-semibold text-info">
 
 <span className="flex items-center gap-2">
 <ScanFace size={16} className="animate-pulse" /> Deep Scanning All Feeds...
 </span>
