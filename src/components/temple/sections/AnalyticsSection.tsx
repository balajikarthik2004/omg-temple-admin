import { BarChart, Bar, AreaChart, Area, LineChart, Line, ResponsiveContainer, XAxis, YAxis, Tooltip, CartesianGrid, Legend } from "recharts";
...
const peakDist = Array.from({ length: 16 }).map((_, i) => ({
  h: `${6 + i}:00`, v: Math.round(800 + Math.sin((i - 3) / 2) * 700 + Math.random() * 600 + (i > 4 && i < 11 ? 2000 : 0)),
}));

const journey = [
  { stage: "Avg", entry: 8, queue: 38, darshan: 6, exit: 5 },
];

const efficiency = Array.from({ length: 12 }).map((_, i) => ({ t: `${i * 10}m`, v: 70 + Math.round(Math.sin(i / 2) * 10) + i }));

const monthly = ["May","Jun","Jul","Aug","Sep","Oct"].map((m, i) => ({ m, v: 240000 + i * 18000 + Math.round(Math.random() * 12000) }));
const satisfaction = ["Wk1","Wk2","Wk3","Wk4","Wk5","Wk6","Wk7","Wk8"].map((w, i) => ({ w, v: 3.8 + Math.random() * 0.5 + i * 0.02 }));
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex gap-2 text-xs">
          {["Today", "7 Days", "30 Days", "Custom"].map((r, i) => (
            <button key={r} className={`rounded-md px-3 py-1.5 ${i === 1 ? "bg-saffron text-white" : "bg-card border border-border"}`}>{r}</button>
          ))}
        </div>
        <button className="rounded-md bg-foreground px-3 py-1.5 text-xs font-medium text-background">Export Report</button>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card title="Peak Hour Distribution">
          <AreaChart data={peakDist}>
            <defs><linearGradient id="pg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--saffron)" stopOpacity={0.5}/><stop offset="100%" stopColor="var(--saffron)" stopOpacity={0}/></linearGradient></defs>
            <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card title="Crowd Satisfaction (avg 4.1/5)">
          <AreaChart data={satisfaction}>
            <defs><linearGradient id="sg2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--gold)" stopOpacity={0.5}/><stop offset="100%" stopColor="var(--gold)" stopOpacity={0}/></linearGradient></defs>
            <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm">
        <div className="border-b border-border px-5 py-3 font-semibold">This Week vs Last Week</div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-surface text-xs uppercase text-muted-foreground">
              <tr>{["Metric", "This Week", "Last Week", "Change"].map(h => <th key={h} className="px-4 py-2 text-left font-medium">{h}</th>)}</tr>
            </thead>
            <tbody className="divide-y divide-border">
              {summary.map(([m, t, l, c, up]) => (
                <tr key={m as string} className="hover:bg-surface">
                  ...
                  <td className={`px-4 py-2 font-semibold tabular-nums ${up ? "text-emerald" : "text-emerald"}`}>{up ? "↑" : "↓"} {String(c).replace("-","")}</td>
                </tr>
              ))}
            </tbody>
          </table>
